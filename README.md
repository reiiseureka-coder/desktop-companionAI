...
現在は通常のチャット機能に加えて、Google Calendar から `【MTG】` / `【移動】` の予定を取得して一覧表示し、5分前にローカル通知する予定アシスト機能を持っています。
...
3. タイトルに `【MTG】` または `【移動】` を含むイベントだけ抽出します。
...
`src/components/ChatWindow.tsx` の `SCHEDULE_TAGS` を変更すると、抽出対象のタグを追加できます。
